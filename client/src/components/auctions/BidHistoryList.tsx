...
export function BidHistoryList({ bids, currentUserId }: { bids: Bid[]; currentUserId: string | undefined }) {
  ...
  const recentBids = bids.slice(0, MAX_VISIBLE);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {bids.length > MAX_VISIBLE && (
        <p className="mt-1 font-[family-name:var(--font-barlow)] text-[11px] text-muted-on-sand">
          + {bids.length - MAX_VISIBLE} earlier bid{bids.length - MAX_VISIBLE === 1 ? "" : "s"}
        </p>
      )}
    </div>
  );
